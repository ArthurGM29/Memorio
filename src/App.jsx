...
import {
  ...
  Grid, FilePlus, Check, Map as MapIcon, CalendarClock, BarChart as BarIcon, Tag, Play, Pause // <--- Adicionado Pause
} from "lucide-react";
...
const TRANSLATIONS = {
  pt: {
    ...
    stats_chart_weekday: "Atividade Semanal",
  ...
  en: {
    ...
    tag_all: "All",
    flashback_title: "On This Day",
    flashback_subtitle: "Relive what happened today in previous years.",
    disc_explore: "EXPLORE",
    disc_map: "Map",
    disc_desc_map: "Visualize your memories around the world.",
    disc_timeline: "Timeline",
    disc_desc_timeline: "Your story in chronological order.",
    disc_stats: "Statistics",
    disc_desc_stats: "Insights about your life and travels.",
    disc_tags: "Tags",
    disc_desc_tags: "Organize and filter by themes.",
    btn_slideshow: "Slideshow",
    stats_total_memories: "Memories",
    stats_total_albums: "Albums",
    stats_total_favs: "Favorites",
    stats_places: "Places",
    stats_chart_country: "Memories by Country",
    stats_chart_tags: "Top Tags",
    stats_chart_weekday: "Weekly Activity",
    stats_chart_timeline: "Timeline Evolution",
    map_loading: "Loading coordinates...",
    map_view_gallery: "View Gallery",
    map_popup_memory: "memory",
    map_popup_memories: "memories",
...
const LocationSelector = ({ country, state, city, onChange }) => {
  ...
  useEffect(() => {
    fetch("https://countriesnow.space/api/v0.1/countries/flag/images")
      .then((res) => res.json()).then((data) => { if (!data.error) setCountriesList(data.data.sort((a, b) => a.name.localeCompare(b.name))); });
  }, []);
  useEffect(() => {
    if (!country) { setStatesList([]); return; }
    const cObj = countriesList.find(c => c.name === country);
    if (cObj) fetch("https://countriesnow.space/api/v0.1/countries/states", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ country: cObj.name }), }).then((res) => res.json()).then((data) => { if (!data.error) setStatesList(data.data.states.sort((a, b) => a.name.localeCompare(b.name))); });
  }, [country, countriesList]);
  useEffect(() => {
    if (!country || !state) { setCitiesList([]); return; }
    fetch("https://countriesnow.space/api/v0.1/countries/state/cities", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ country, state }), }).then((res) => res.json()).then((data) => { if (!data.error) setCitiesList(data.data.sort((a, b) => a.localeCompare(b))); });
  }, [state, country]);
  return (
    <div className="location-grid">
      <div><label className="editor-label">País</label><div style={{ position: "relative" }}><input list="countries-data" placeholder="Selecione..." value={country} onChange={(e) => onChange("country", e.target.value)} required /><datalist id="countries-data">{countriesList.map((c, idx) => (<option key={idx} value={c.name} />))}</datalist></div></div>
      <div><label className="editor-label">Estado</label><input list="states-data" placeholder={country ? "Selecione..." : "..."} value={state} onChange={(e) => onChange("state", e.target.value)} disabled={!country} /><datalist id="states-data">{statesList.map((s, idx) => (<option key={idx} value={s.name} />))}</datalist></div>
      <div><label className="editor-label">Cidade</label><input list="cities-data" placeholder={state ? "Selecione..." : "..."} value={city} onChange={(e) => onChange("city", e.target.value)} disabled={!state} /><datalist id="cities-data">{citiesList.map((c, idx) => (<option key={idx} value={c} />))}</datalist></div>
    </div>
  );
};
      ...
      <div className="carousel-track" ref={scrollRef}>
        {memories.map((mem, i) => (
          <div key={mem.id} className="polaroid-card" onClick={() => onSelect(mem)} style={{transform: `rotate(${(i%2===0?1:-1)*(Math.random()*1)}deg)`}}>
             ...
             <div className="polaroid-meta">{new Date(mem.date).toLocaleDateString()}{mem.location && <span> • {mem.location.split(',')[0]}</span>}</div>
...
// --- NOVO COMPONENTE DE SLIDESHOW (MODO CINEMA) ---
const SlideshowModal = ({ isOpen, onClose, memories }) => {
  ...
  useEffect(() => {
    if (isOpen && isPlaying && memories.length > 0) {
      ...
      intervalRef.current = setInterval(() => {
        ...
        const p = Math.min((elapsed / 5000) * 100, 100); // 5 segundos por slide
        setProgress(p);
  ...
  return (
    <div className="slideshow-overlay">
      <div className="ss-progress-wrapper">
        <div className="ss-progress-bar" style={{ width: `${progress}%` }} />
      </div>
      ...
      <div className="slideshow-media-container" onClick={(e) => e.stopPropagation()}>
        {memories.map((mem, i) => (
          (i === index) && (
            mem.mediaType === 'video' ? 
            <video key={mem.id} src={mem.media} className="slideshow-media active" autoPlay loop muted /> 
            : 
            <img key={mem.id} src={mem.media} className="slideshow-media active" alt={mem.title} />
          )
        ))}
      </div>
      ...
      <button className="ss-btn play-toggle" onClick={(e) => { e.stopPropagation(); setIsPlaying(!isPlaying); }}>
...
const AddOptionModal = ({ isOpen, onClose, onCreateNew, onSelectExisting, t }) => {
  if (!isOpen) return null;
  return (
    <div className="modal-overlay" onClick={onClose}><div className="modal-box" onClick={e => e.stopPropagation()}><h3 className="modal-title">{t('add_to_album_title')}</h3><div className="choice-grid"><div className="choice-card" onClick={onCreateNew}><div className="choice-icon"><FilePlus size={28} /></div><div className="choice-label">{t('option_create_new')}</div></div><div className="choice-card" onClick={onSelectExisting}><div className="choice-icon"><Grid size={28} /></div><div className="choice-label">{t('option_select_existing')}</div></div></div><button onClick={onClose} className="btn-modal-cancel" style={{marginTop: '2rem'}}>{t('btn_cancel')}</button></div></div>
  );
};
  ...
  return (
    <div className="container fade-in" style={{paddingTop: '2rem'}}>
                ...
                <Popup>
                  <div style={{textAlign:'center', minWidth: 150}}>
                    ...
                    <div style={{marginBottom:10, fontSize:'0.85rem', color:'#666'}}>
                      {memoriesAtPlace.length} {memoriesAtPlace.length === 1 ? t('map_popup_memory') : t('map_popup_memories')}
                    </div>
                    <button 
                      onClick={() => setSelectedLocation({name: loc.name, list: memoriesAtPlace})} 
      ...
      <LocationListModal 
        ...
        memories={selectedLocation?.list || []} 
        onSelect={(m) => {
            onSelect(m); 
        }}
        t={t}
...
const TimelineView = ({ memories, onSelect, onBack, t }) => {
  ...
  return (
    <div className="container fade-in" style={{paddingTop: '2rem'}}>
      ...
      <div className="timeline-container">{sorted.map((mem, i) => (<div key={mem.id} className={`timeline-item ${i % 2 !== 0 ? 'right' : ''}`}><div className="timeline-dot"></div><div className="timeline-content" onClick={() => onSelect(mem)}><span className="timeline-date">{new Date(mem.date).toLocaleDateString()}</span><h4 style={{margin:0, fontFamily:'Cinzel'}}>{mem.title}</h4><div style={{width:'100%', height:150, marginTop:10, borderRadius:4, overflow:'hidden'}}>{mem.mediaType==='video' ? <video src={mem.media} style={{width:'100%', height:'100%', objectFit:'cover'}}/> : <img src={mem.media} style={{width:'100%', height:'100%', objectFit:'cover'}}/>}</div></div></div>))}</div>
    </div>
  );
};
...
const StatsView = ({ memories, albums, onBack, t, theme, lang }) => {
  ...
  const tagsData = Object.keys(tagsCount)
    .map(k => ({ name: k, value: tagsCount[k] }))
    .sort((a,b) => b.value - a.value)
    .slice(0, 5);

  const daysPT = ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'];
  const daysEN = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
  const daysOfWeek = lang === 'en' ? daysEN : daysPT;
  ...
  const weekData = weekCount.map((v, i) => ({ name: daysOfWeek[i], count: v }));

  const timelineData = memories.reduce((acc, m) => {
      const d = new Date(m.date);
      ...
      <div className="charts-grid">
              ...
              <div style={{height: 300}}>
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={countryData}>
                        ...
                        <XAxis dataKey="name" stroke="var(--text-muted)" style={{fontSize: 12}} />
                        <YAxis stroke="var(--text-muted)" />
                        <Tooltip contentStyle={{backgroundColor: 'var(--bg-glass)', borderRadius: 8, border: '1px solid var(--primary)'}} itemStyle={{color: 'var(--text-main)'}} />
          ...
          <div className="chart-box">
              ...
              <div style={{height: 300}}>
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        <Pie data={tagsData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                            {tagsData.map((entry, index) => (
                                <Cell key={`cell-${index}`} fill={activePalette[index % activePalette.length]} />
                            ))}
                        </Pie>
                        <Tooltip contentStyle={{backgroundColor: 'var(--bg-glass)', borderRadius: 8, border: '1px solid var(--primary)'}} />
                        <Legend />
                    </PieChart>
                </ResponsiveContainer>
              </div>
          ...
          <div className="chart-box">
              <h3 className="chart-title">{t('stats_chart_weekday')}</h3>
              <div style={{height: 300}}>
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={weekData}>
                        ...
                        <YAxis stroke="var(--text-muted)" />
                        <Tooltip contentStyle={{backgroundColor: 'var(--bg-glass)', borderRadius: 8, border: '1px solid var(--primary)'}} itemStyle={{color: 'var(--text-main)'}} />
                        <Bar dataKey="count" fill={activePalette[2]} radius={[4, 4, 0, 0]} />
                    </BarChart>
                </ResponsiveContainer>
              </div>
          </div>
...
const TagsView = ({ memories, onSelect, onBack, t }) => {
  ...
  return (
    <div className="container fade-in" style={{paddingTop: '2rem'}}>
      ...
      <div className="grid-polaroid">{filtered.map((mem, i) => (<div key={mem.id} className="polaroid-card" onClick={() => onSelect(mem)}><img src={mem.media} className="polaroid-img" alt={mem.title}/><div className="polaroid-caption">{mem.title}</div></div>))}</div>
    </div>
  );
};

// ==================================================================================
// 5. COMPONENTES PRINCIPAIS DE VIEW (DASHBOARD, FAVORITES, ALBUMS)
// ==================================================================================
...
const FavoritesView = ({ memories, onSelect, onNew, onViewAlbums, onViewHome, onViewFavorites, t, settingsProps }) => {
  ...
  return (
    <div style={{ minHeight: '100vh', paddingBottom: 50 }}>
      ...
      <div className="container fade-in" style={{paddingTop: '3rem'}}><div className="section-divider"><div className="divider-line"></div><h2 className="section-title">{t('my_favorites')}</h2><p className="section-subtitle">{favorites.length} {t('empty_album')}</p></div>{filteredFavorites.length === 0 ? (<div style={{textAlign:'center', color:'var(--text-muted)', marginTop:'3rem'}}><Heart size={48} style={{opacity:0.2, marginBottom:10}}/><p>Você ainda não tem favoritos.</p></div>) : (<div className="grid-polaroid">{filteredFavorites.map((mem, i) => (<div key={mem.id} className="polaroid-card" onClick={() => onSelect(mem)} style={{transform: `rotate(${(i%2===0?1:-1)*(Math.random()*2)}deg)`}}><img src={mem.media} className="polaroid-img" alt={mem.title}/><div className="polaroid-caption">{mem.title}</div><div className="polaroid-meta">{new Date(mem.date).toLocaleDateString()}</div><Heart size={16} fill="var(--primary)" color="var(--primary)" style={{position:'absolute', top:10, right:10}}/></div>))}</div>)}</div>
    </div>
  );
};
...
const MemoryForm = ({ onSave, onCancel, initialData, albums, preSelectedAlbumId, t }) => {
  ...
  const [fd, setFd] = useState(initialData || { title:"", date: new Date().toISOString().split("T")[0], description:"", story:"", location:"", isFavorite:false, media:null, mediaType:"image", albumId: preSelectedAlbumId||"", tags: [] });
...
function App() {
  ...
  // ESTADO ELEVADO DO MAPA (Para persistir o modal aberto ao voltar do detalhe)
  ...
  useEffect(() => {
    ...
    const processedMemories = (m.length ? m : [INITIAL_MEMORY]).map(mem => ({...mem, tags: mem.tags || []}));
    ...
    <div className="app-wrapper">
      ...
      {!user ? <Auth onLogin={handleLogin} t={t} /> : (
        <>
          ...
          {view === "album-detail" && currentAlbum && (
             <div className="container fade-in" style={{paddingTop: '2rem'}}>
                 ...
                 <div className="grid-polaroid">
                    {displayedMemories.map(mem => (<div key={mem.id} className="polaroid-card" onClick={()=> {setLastView("album-detail"); setSelectedMemory(mem); setView('detail')}}><img src={mem.media} className="polaroid-img" /><div className="polaroid-caption">{mem.title}</div></div>))}
                 </div>
             </div>
          )}
          ...
          {/* PASSA O TEMA E LINGUAGEM PARA STATS VIEW */}
